...
import { AuthState, ClientState, useTurnkey, WalletSource, type EmbeddedWallet, type TurnkeySDKClientBase } from "@turnkey/react-wallet-kit";
import type {
  WalletData,
} from '@eth-optimism/verbs-service'
import NavBar from './NavBar'
import { TurnkeyAuthButton } from './PrivyAuthButton'
...
import { SUPPORTED_TOKENS, type SmartWallet, type SupportedChainId, type TokenBalance } from '@eth-optimism/verbs-sdk/react';
...
const Terminal = () => {
  ...
  // Privy wallet hooks
  const { wallets, clientState, authState, user, createWallet, refreshWallets, httpClient, session  } = useTurnkey();
  ...
  useEffect(() => {
    ...
    if (clientState === ClientState.Ready && authState === AuthState.Authenticated && !embeddedWallet) {
  ...
  useEffect(() => {
      ...
      if (
        ...
        session?.organizationId
          ...
          const initializeVerbsWallet = async () => {
            ...
            console.log('session!.organizationId', session.organizationId)
            const verbsWallet = await verbs.wallet.hostedWalletToVerbsWallet({client: httpClient, organizationId: session.organizationId, signWith: embeddedWallet.accounts[0].address, ethereumAddress: embeddedWallet.accounts[0].address})
    ...
    autoSelectWallet()
  }, [wallets.length, selectedWallet, user, embeddedWallet, authState, clientState])
      ...
      vaults.map(async (vault) => {
        try {
          const vaultBalance = await wallet.lend!.getPosition(
            { marketId: { address: vault.address, chainId: vault.chainId as SupportedChainId } },
          )
...
How much would you like to lend?`
  ...
  const handleLendAmountSubmission = async (amount: number) => {
    ...
    try {
      ...
      const depositHash = await selectedWallet!.lend!.openPosition({
        amount: amount,
        asset: asset,
        marketId: { address: promptData.selectedVault.address as Address, chainId: promptData.selectedVault.chainId as SupportedChainId },
      })
    const innerResult = {
      hash: depositHash,
    ...
    const result = {
      transaction: {
        ...
        asset: asset,
        marketId: { address: promptData.selectedVault.address as Address, chainId: promptData.selectedVault.chainId as SupportedChainId },
        apy: promptData.selectedVault.apy,
      },
    }
...
Tx:     ${result.transaction.blockExplorerUrl}/${result.transaction.hash || 'pending'}`,
  ...
  return (
    <div
      ...
      onClick={handleClick}
    >
      <NavBar fullWidth rightElement={<TurnkeyAuthButton />} />
